...
class VideoTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        if self.checkpoint:
            return checkpoint(self._forward, x, context, timesteps, use_reentrant=False) # unsure: use_reentrant=False
        else:
    ...
    def _forward(self, x, context=None, timesteps=None):
        ...
        if self.disable_self_attn:
            ...
            x = self.attn1(self.norm1(x), align_w_first_frame=True) + x

        if self.attn2 is not None:
            # print(self.switch_temporal_ca_to_sa)
