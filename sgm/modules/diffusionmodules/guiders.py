...
class LinearPredictionGuider(Guider):
    def __init__(
        ...
        self.scale = torch.linspace(min_scale, max_scale, num_frames).unsqueeze(0)#.flip(dims=(1,))
    ...
    def __call__(self, x: torch.Tensor, sigma: torch.Tensor) -> torch.Tensor:
        ...
        scale = append_dims(scale, x_u.ndim).to(x_u.device, dtype=x_u.dtype)
...
class LinearPredictionGuider_fp16(Guider):
    def __init__(
        self,
        max_scale: float,
        num_frames: int,
    ...
    def __call__(self, x: torch.Tensor, sigma: torch.Tensor) -> torch.Tensor:
        ...
        x_c = rearrange(x_c, "(b t) ... -> b t ...", t=self.num_frames)
        scale = repeat(self.scale, "1 t -> b t", b=x_u.shape[0])
        scale = append_dims(scale, x_u.ndim).to(x_u.device, dtype=x.dtype)
